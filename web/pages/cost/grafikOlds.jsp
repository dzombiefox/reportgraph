...
<%
  ...
  out.print("<h4 align='center'>Graphic Production "+nline+" &nbsp;"+bulan+"&nbsp;"+y+" Real Nyata</h4>");
   ...
   String query="select * from reportOld join line on reportOld.line_id=line.line_id where line.line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc ";
...
<table   data-auto-width="false" id="example" style='border :1px solid black;' > 
    <%   
String quer="select * from reportOld join line on reportOld.line_id=line.line_id where line.line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
...
while(rs2.next()){
    ...
    out.print("<td style='border-right:1px solid black'><table  class='in'><tr><td align='right' class='satu'>"+nf.format(sch)+"</td><td align='right'>(100%)</td></tr></table></td>");
}
String tsch="select sum(sch)as tot from reportOld join line on reportOld.line_id=line.line_id where line.line_id='"+li+"' and date between '"+begin+"' and '"+end+"'";
ResultSet rsch=c.GetData(tsch);
rsch.next();
 Double stot=rsch.getDouble("tot");
...
String tqtyt="select SUM(kw1)+SUM(kw2)+SUM(kw3)+SUM(kw4)+SUM(kw5) as total from reportOld  join line on reportOld.line_id=line.line_id where line.line_id='"+li+"' and date between '"+begin+"' and '"+end+"'";
String cekKw="select sum(kw1)as kw1,sum(kw2)as kw2,sum(kw3)as kw3,sum(kw4)as kw4,sum(kw5)as kw5 from reportOld join line on reportOld.line_id=line.line_id where line.line_id='"+li+"' and date between '"+begin+"' and '"+end+"'";
...
String valSch="select sum(x) as total from(select (sum(sch)*(hpp_qty1))/1000000 as x,sum(sch)as schedule,sell_qty1 as hargaJual,hpp_qty1 as hpp1 from reportOld  join line on reportOld.line_id=line.line_id where line.line_id='"+li+"'  and date between '"+begin+"' and '"+end+"'  group by sch,hpp_qty1,sell_qty1)a";
ResultSet rVal=c.GetData(valSch);
rVal.next();
Float dVal=rVal.getFloat("total");
...
String vre="select SUM(qty)as total from(select ((kw1*hpp_qty1)+(kw2*hpp_qty2)+(kw3*hpp_qty3)+(kw4*hpp_qty4)+(kw5*hpp_qty5))/1000000 as qty from reportOld where date between '"+begin+"' and '"+end+"' and line_id='"+li+"' )a";
...
String proSch="select sum(x)as total from(select(sum(sch)*(sell_qty1-hpp_qty1))/1000000 as x,sum(sch)as schedule,hpp_qty1 as hppqty,sell_qty1 as hargaJual,hpp_qty1 as hpp1 from reportOld  join line on reportOld.line_id=line.line_id where line.line_id='"+li+"'  and date between '"+begin+"' and '"+end+"'  group by sch,hpp_qty1,sell_qty1,hpp_qty1)a";
ResultSet rPro=c.GetData(proSch);
rPro.next();
Float dPro=rPro.getFloat("total");
...
String proReal="select SUM(qty)as total from(select (kw1*(sell_qty1-hpp_qty1)+kw2*(sell_qty2-hpp_qty2)+kw3*(sell_qty3-hpp_qty3)+kw4*(sell_qty4-hpp_qty4)+kw5*(sell_qty5-hpp_qty5))/1000000 as qty from reportOld where date between '"+begin+"' and '"+end+"' and line_id='"+li+"' )a";
...
DefaultCategoryDataset data = new DefaultCategoryDataset();
 String x="select * from reportOld join line on reportOld.line_id=line.line_id where line.line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
