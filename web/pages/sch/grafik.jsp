...
<body>
 <div id="mydiv" >
  ...
  out.print("<h4 align='center'>Graphic Production "+nline+" &nbsp;"+monthString+"&nbsp;"+y+" Real Nyata</h4>");
    ...
    String query="select((b.sch*isnull(b.pkw2,0)/100)*(sell_kw2-hpp_kw2)+(b.sch*isnull(b.pkw3,0)/100)*(sell_kw3-hpp_kw3)+(b.sch*isnull(b.pkw4,0)/100)*(sell_kw4-hpp_kw4)+(b.sch*isnull(b.pkw5,0)/100)*(sell_kw5-hpp_kw5))/1000000 as proSch,((kw1*(sell_kw1-hpp_kw1))+(kw2*(sell_kw2-hpp_kw2))+(kw3*(sell_kw3-hpp_kw3))+(kw4*(sell_kw4-hpp_kw4))+(kw5*(sell_kw5-hpp_kw5)))/1000000 as proReal,((b.sch*isnull(b.pkw1,0)/100*hpp_kw1)+(b.sch*isnull(b.pkw2,0)/100*hpp_kw2)+(b.sch*isnull(b.pkw3,0)/100*hpp_kw3)+(b.sch*isnull(b.pkw4,0)/100*hpp_kw4)+(b.sch*isnull(b.pkw5,0)/100*hpp_kw5) )/1000000 as valSch,((kw1*hpp_kw1)+(kw2*hpp_kw2)+(kw3*hpp_kw3)+(kw4*hpp_kw4)+(kw5*hpp_kw5))/1000000 as valReal  from sch s join  prices p on s.price_id=p.prices_id join persen b on p.persen_id=b.persen_id    where line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
      ...
      <%
       String queryDate="select * from sch join prices on sch.price_id=prices.prices_id join persen on prices.persen_id=persen.persen_id join line on persen.line_id=line.line_id where line.line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
         ...
         while(rdate.next()){
            ...
            out.print("</tr>");   
            out.print("<tr >");
       String querySch="select ((p.sch*p.pkw1)+(p.sch*p.pkw2)+(p.sch*p.pkw3)+(p.sch*p.pkw4)+(p.sch*p.pkw5))/100 as hasil from sch a join prices b on a.price_id=b.prices_id join persen p on b.persen_id=p.persen_id  where line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
          ResultSet rsch=c.GetData(querySch);
            ...
            String queryQtyReal="select ((b.sch*b.pkw1)+(b.sch*b.pkw2)+(b.sch*b.pkw3)+(b.sch*b.pkw4)+(b.sch*b.pkw5))/100 as sch,a.kw1+a.kw2+a.kw3+a.kw4+a.kw5 as qty from sch a join prices p on a.price_id=p.prices_id join persen b on b.persen_id=p.persen_id where line_id='"+li+"' and date between '"+begin+"' and '"+end+"'  order by date asc";
            ...
            String queryValsch="select (((b.sch*isnull(b.pkw1,0)/100)*hpp_kw1)+((b.sch*isnull(b.pkw2,0)/100)*hpp_kw2)+((b.sch*isnull(b.pkw3,0)/100)*hpp_kw3)+((b.sch*isnull(b.pkw4,0)/100)*hpp_kw4)+((b.sch*isnull(b.pkw5,0)/100)*hpp_kw5) )/1000000 as hasil from sch a join prices p on a.price_id=p.prices_id join  persen b on p.persen_id=b.persen_id where line_id='"+li+"' and date between '"+begin+"' and '"+end+"'  order by date asc";
            ...
            while(rvalsch.next()){
                ...
                if(rvalsch.getDouble("hasil")==0){
                    angka=String.format("%.0f",rvalsch.getDouble("hasil"));
                }else{
            ...
            String queryValReal="select (((b.sch*isnull(b.pkw1,0)/100)*hpp_kw1)+((b.sch*isnull(b.pkw2,0)/100)*a.hpp_kw2)+((b.sch*isnull(b.pkw3,0)/100)*hpp_kw3)+((b.sch*isnull(b.pkw4,0)/100)*a.hpp_kw4)+((b.sch*isnull(b.pkw5,0)/100)*hpp_kw5) )/1000000 as valSch,(s.kw1*a.hpp_kw1)+(s.kw2*a.hpp_kw2)+(s.kw3*a.hpp_kw3)+(s.kw4*hpp_kw4)+(s.kw5*a.hpp_kw5)as hasil,(((b.sch*b.pkw1)+(b.sch*b.pkw2)+(b.sch*b.pkw3)+(b.sch*b.pkw4)+(b.sch*b.pkw5))/100)/ISNULL(NULLIF(hpp_kw1,0),1) as banding from  sch s join prices a on s.price_id=a.prices_id join persen b on a.persen_id=b.persen_id where line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
            ResultSet rvalreal=c.GetData(queryValReal);
		        ...
		        bandingSch=rvalreal.getDouble("valSch");
                        ...
                        else if((valReal>0)&&(valReal<bandingSch)){
                             font="#003399";
            ...
            out.print("<td style='border-top:1px solid #D0D0D0'>Variant</td>");
                String queryVariantVal="select (((kw1*hpp_kw1)+(kw2*hpp_kw2)+(kw3*hpp_kw3)+(kw4*hpp_kw4)+(kw5*hpp_kw5)))/1000000-(((b.sch*isnull(b.pkw1,0)/100)*hpp_kw1)+((b.sch*isnull(b.pkw2,0)/100)*hpp_kw2)+((b.sch*isnull(b.pkw3,0)/100)*hpp_kw3)+((b.sch*isnull(b.pkw4,0)/100)*hpp_kw4)+((b.sch*isnull(b.pkw5,0)/100)*hpp_kw5))/1000000 as variant from  sch s join prices a on s.price_id=a.prices_id join persen b on a.persen_id=b.persen_id  where line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
            ...
            out.print("<td align='right' style='border-left:1px solid black  ;border-top:1px solid #D0D0D0'><table  class='in' ><tr><td align='right' class='satu'><font style='color:"+fontVariant+"'>"+totalVarian+"</font></td><td align='right'>("+((int)Math.round(((totalValReal/(valTotsch))*100))-100)+")%</td></tr></table></td>");
            ...
            String queryvalSch="select (((b.sch*isnull(b.pkw1,0)/100)*(sell_kw1-hpp_kw1))+((b.sch*isnull(b.pkw2,0)/100)*(sell_kw2-hpp_kw2))+((b.sch*isnull(b.pkw3,0)/100)*(sell_kw3-hpp_kw3))+((b.sch*isnull(b.pkw4,0)/100)*(sell_kw4-hpp_kw4))+((b.sch*isnull(b.pkw5,0)/100)*(sell_kw5-hpp_kw5)))/1000000 as proSch from  sch s join prices a on s.price_id=a.prices_id join persen b on a.persen_id=b.persen_id where line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
            ResultSet rvalSch=c.GetData(queryvalSch);
            ...
            String queryProReal="select (((p.sch*isnull(p.pkw1,0)/100)*(b.sell_kw1-b.hpp_kw1))+((p.sch*isnull(p.pkw2,0)/100)*(b.sell_kw2-b.hpp_kw2))+((p.sch*isnull(p.pkw3,0)/100)*(b.sell_kw3-b.hpp_kw3))+((p.sch*isnull(p.pkw4,0)/100)*(b.sell_kw4-b.hpp_kw4))+((p.sch*isnull(p.pkw5,0)/100)*(b.sell_kw5-b.hpp_kw5)))/1000000 as proSch ,((a.kw1*(b.sell_kw1-b.hpp_kw1))+(a.kw2*(b.sell_kw2-b.hpp_kw2))+(a.kw3*(b.sell_kw3-b.hpp_kw3))+(a.kw4*(b.sell_kw4-b.hpp_kw4))+(a.kw5*(b.sell_kw5-b.hpp_kw5)))/1000000 as pro from  sch a join prices b on a.price_id=b.prices_id join persen p on b.persen_id=p.persen_id where line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
		    ...
		    Double band=rqueryPro.getDouble("proSch");                   
                    ...
                    else if(valDataPro==0){
                        replace=String.format("%.0f", valDataPro);
                         f="black";
                    }
            ...
            out.print("<tr >");
            out.print("<td  style='border-top:1px solid #D0D0D0;'>Variant</td>");
                String varianPro="select (((a.kw1*(b.sell_kw1-b.hpp_kw1))+(a.kw2*(b.sell_kw2-b.hpp_kw2))+(a.kw3*(b.sell_kw3-b.hpp_kw3))+(a.kw4*(b.sell_kw4-b.hpp_kw4))+(a.kw5*(b.sell_kw5-b.hpp_kw5)))/1000000)-((((p.sch*isnull(p.pkw1,0)/100)*(sell_kw1-hpp_kw1))+((p.sch*isnull(p.pkw2,0)/100.)*(sell_kw2-hpp_kw2))+((p.sch*isnull(p.pkw3,0)/100)*(sell_kw3-hpp_kw3))+((p.sch*isnull(p.pkw4,0)/100)*(sell_kw4-hpp_kw4))+((p.sch*isnull(p.pkw5,0)/100)*(sell_kw5-hpp_kw5)))/1000000) as variant from  sch a join prices b on a.price_id=b.prices_id join persen p on b.persen_id=p.persen_id  where line_id='"+li+"' and date between '"+begin+"' and '"+end+"' order by date asc";
            ...
            out.print("<td align='right' style='border-left:1px solid black  ;border-top:1px solid #D0D0D0'><table  class='in' ><tr><td align='right' class='satu'><font style='color:"+fo+"'>"+totVar+"</font></td><td align='right'>("+(((int)Math.round(((totalProReal/totalValSch)*100)))-100)+")%</td></tr></table></td>");
            out.print("<td style='border-top:1px solid #D0D0D0;border-bottom:1px solid black'>Variant</td>");
            out.print("</tr >");
        ...
        JFreeChart cha=ChartFactory.createLineChart("", "Date", "M2", data,PlotOrientation.VERTICAL, true,true,true);
          ...
          //  plots.setRenderer(renderer2);
            ...
            plots.setRangeGridlinePaint(Color.black);
              LegendTitle legend2 = cha.getLegend();
                legend2.setItemFont(new Font("Dialog", Font.PLAIN, 18));
                legend2.setFrame(new BlockBorder(Color.black));
