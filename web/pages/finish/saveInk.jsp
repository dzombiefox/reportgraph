...
String que="insert into finish(date,item_code,motif_id,size_id,periode_id,color_id,opt,body_id,alumina_id,engobe_id,glaze_id,dfinish_id)values"
        + "('"+tanggal+"','"+code+"','"+f.getMotif()+"','"+f.getSize()+"','"+f.getPeriode()+"','"+f.getColor()+"','"+f.getOpt()+"','"+f.getBody()+"','"+f.getAlumina()+"','"+f.getEngobe()+"','"+f.getGlaze()+"','"+finish+"')";
c.ManipulasiData(que);

for(int i=0;i<=row;i++){
JSONArray ar= (JSONArray) jsonObj.get("dataInks");
JSONObject fr = (JSONObject) ar.get(i);
String id=fr.get("id").toString();
String query="insert into dink(dfinish_id,ink_id)values('"+finish+"','"+id+"')";
